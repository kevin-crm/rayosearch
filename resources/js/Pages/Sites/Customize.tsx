        ...
        <div style={{ background: c.bg, padding: '20px', borderRadius: '10px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
            ...
            {results !== null && results.length > 0 && (
                <div style={{ marginTop: '10px', background: config.bgColor || c.panel, border: `1px solid ${c.border}`, borderRadius: radius, overflow: 'hidden' }}>
                    ...
                    {/* Card — horizontal media card with accent stripe */}
                        ...
                        <div style={{ padding: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {results.map((r, i) => (
                                <div key={r.id || i} style={{
                                    ...
                                    border: `1px solid ${c.border}`, borderLeft: `3px solid ${config.accent}`,
                                    borderRadius: radius,
                                }}>
                            ...
                            {results.map((r, i) => (
                                <div key={r.id || i} style={{ background: c.bg, border: `1px solid ${c.border}`, borderRadius: radius, overflow: 'hidden' }}>
                                    ...
                                    <div style={{ padding: '10px 12px' }}>
                                        ...
                                        {r.url && (
                                            <div style={{ fontSize: '10px', color: config.accent, marginTop: '6px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                                {r.url}
                                            </div>
                                        )}
                                    </div>
                        ...
                        <div style={{ padding: '10px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '10px' }}>
                            {results.map((r, i) => (
                                <a key={r.id || i} href={r.url || undefined} target="_blank" rel="noopener noreferrer" style={{ background: c.bg, border: `1px solid ${c.border}`, borderRadius: radius, overflow: 'hidden', display: 'flex', flexDirection: 'column', textDecoration: 'none', color: 'inherit' }}>
                                    <div style={{ width: '100%', aspectRatio: '1/1', overflow: 'hidden', position: 'relative', flexShrink: 0 }}>
                                        {r.image ? (
                                            <img
                                                src={r.image}
                                                alt={r.title}
...
export default function Customize({ site }: Props) {
    const [config, setConfig] = useState<WidgetConfig>(site.widget_config ?? DEFAULT_CONFIG)
    ...
    const save = async () => {
        ...
        setSaveMsg(null)
        try {
            const res = await fetch(`/sites/${site.site_id}/widget-config`, {
                method: 'POST',
                headers: {
    ...
    return (
        <AppLayout title={`Customize — ${site.name}`}>
            <div className="flex flex-col gap-5 h-full">
                ...
                <div className="flex items-center gap-3">
                    <Link
                        ...
                        className="text-xs transition-colors hover:text-[#5aa9ff]"
                        style={{ color: 'var(--fg-muted)' }}
                    >
                        ← {site.name}
                    </Link>
                    <span style={{ color: 'var(--border)' }}>/</span>
                    <span className="text-xs font-medium" style={{ color: 'var(--fg)' }}>Widget Builder</span>
                </div>
                    ...
                    <div className="flex-1 flex flex-col gap-3 min-w-0">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="text-[11px]" style={{ color: 'var(--fg-muted)' }}>
                                Searching <span className="font-medium" style={{ color: 'var(--fg)' }}>{site.azure_index_name}</span>
                            </span>
                        </div>
                        ...
                        <div className="flex-1 rounded-lg border overflow-hidden flex flex-col" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--panel)' }}>
                            ...
                            {/* Page mockup — full width, no max-w cap */}
                            <div className="flex-1 overflow-auto p-6" style={{ backgroundColor: config.theme === 'light' ? '#f9fafb' : config.theme === 'dark' ? '#0a0a0a' : 'var(--bg)' }}>
                                ...
                                {/* Widget at full width of mock page */}
                                <WidgetPreview config={config} siteId={site.site_id} />
                            </div>
                        </div>
